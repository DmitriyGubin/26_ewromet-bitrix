...
$this_sec = Return_All_Sections(
        Array("IBLOCK_ID"=>18, "ACTIVE"=>"Y", "ID" => $arResult["ID"]),
        Array('ID','NAME','UF_KEY_PROP','UF_KEY_TITLE')
    );

$GLOBALS['KEY_PROP'] = $this_sec[0]['UF_KEY_PROP'];
$GLOBALS['KEY_TITLE'] = $this_sec[0]['UF_KEY_TITLE'];
...
{
	?>
	<div class="hide bx-section-desc bx-<?=$arParams['TEMPLATE_THEME']?>">
...
<div class="table_head">
	<div class="<?= ($prop_cod == '') ? 'no_prop_col' : 'one_col'; ?>">
		...
		<div class="sort_box sort <?= ($_GET["sort"] == "name") ? 'active' : null; ?>">
			...
			<div class="refs_box hide">
				<a class="prop_asc" href="<?= $page; ?>?sort=name&method=asc<?= $add_str; ?>"></a>
				<a class="prop_desc" href="<?= $page; ?>?sort=name&method=desc<?= $add_str; ?>"></a>
			</div>
		</div>

	</div>

	<div class="add_col <?= ($prop_cod == '') ? 'hide' : null; ?>">
		<?php
			$rule = (($_GET["method"] == 'asc') and ($_GET["sort"] == "property_".$prop_cod)); 
		?>
		<div class="sort_box sort <?= ($_GET["sort"] == "property_".$prop_cod) ? 'active' : null; ?>">
			<div class="small_title">
				<?= $GLOBALS['KEY_TITLE']; ?> 
			</div>
			<img class="filt_icon <?= $rule ? null : 'active'; ?>" src="/catalog/img/sort.svg">
			<div class="refs_box hide">
				<a class="prop_asc" href="<?= $page; ?>?sort=property_<?= $prop_cod; ?>&method=asc"></a>
				<a class="prop_desc" href="<?= $page; ?>?sort=property_<?= $prop_cod; ?>&method=desc"></a>
			</div>
		</div>
	</div>
	...
	<div class="two_col">
		...
		<div class="sort_box sort <?= ($_GET["sort"] == "property_BREND") ? 'active' : null; ?>">
			...
			<div class="refs_box hide">
				<a class="prop_asc" href="<?= $page; ?>?sort=property_BREND&method=asc<?= $add_str; ?>"></a>
				<a class="prop_desc" href="<?= $page; ?>?sort=property_BREND&method=desc<?= $add_str; ?>"></a>
			</div>
		</div>
	</div>
	...
	<div class="three_col">
		...
		<div class="sort_box sort <?= ($_GET["sort"] == "catalog_PRICE_1") ? 'active' : null; ?>">
			...
			<div class="refs_box hide">
				<a class="prop_asc" href="<?= $page; ?>?sort=catalog_PRICE_1&method=asc<?= $add_str; ?>"></a>
				<a class="prop_desc" href="<?= $page; ?>?sort=catalog_PRICE_1&method=desc<?= $add_str; ?>"></a>
			</div>
		</div>
	</div>
...
<div class="pager_box">
	...
	<?php if($showLazyLoad): ?>
		<button class="more_items click">Показать еще</button>
	<?php endif; ?>
</div>
...
{
	?>
	<button class="more_items hide_this hide" data-use="show-more-<?=$navParams['NavNum']?>">Показать еще</button>
	<?
}
...
/*************сортировка*************/
/*для ajax
$sort_boxes = $('.sort_box.sort');

$sort_boxes.on('click', function(){

	$sort_boxes.removeClass('active');
	$(this).addClass('active');

	var filt_icon = $(this).find('.filt_icon');

    filt_icon.toggleClass('active');

    if(filt_icon.hasClass('active'))
    {
...
$('.sort_box.sort').on('click', function(){
    ...
    if(filt_icon.hasClass('active'))
    {
    	$(this).find('.prop_desc')[0].click();
    }
    else
    {
    	$(this).find('.prop_asc')[0].click();
    }
...
$('.more_items.click').on('click', function(){

	$('.more_items.hide_this')[0].click();
